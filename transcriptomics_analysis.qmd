...
### 30 genes that are most up and down regulated and are siginificant ###
```{r}
# determine the upregulated and downregulated genes 
significant_genes <- transcriptomics_data[transcriptomics_data$padj <= 0.05, ]


upregulated_genes <- significant_genes[order(significant_genes$log2FoldChange, decreasing = TRUE), ][1:30, ]
downregulated_genes <- significant_genes[order(significant_genes$log2FoldChange, decreasing = FALSE), ][1:30, ]
...
ggplot(transcriptomics_data, aes(x = log2FoldChange, y = -log10(padj))) +
  ...
  scale_size_continuous(name = "Log FC SE", range = c(0.3, 3)) +  geom_vline(xintercept = c(-1, 1), linetype = "dashed", color = "black") + geom_hline(yintercept = -log10(0.05), linetype = "dashed", color = "black") + theme_minimal() +  labs(title = "Volcano Plot", x = "log2(Fold Change)", y = "-log10(Adjusted p-value)") + theme(plot.title = element_text(hjust = 0.5))  
```
